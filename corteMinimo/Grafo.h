...
{
    if (vertices_disponibles.size() < 2) {
        cout << "No hay suficientes vertices para elegir dos al azar." << endl;
    }
    else {
        ...
        T v2;
        do {
            v2 = elegirDeVector(vertices_disponibles);
        } while (v2 == v1);

        cout << "azar v1: " << v1 << " --> azar v2: " << v2 << endl;
    }
}
...
int Grafo<T>::corteMinimoRapido() {
    ...
    while (copia_lista.size() > 2) {
        ...
        if (v1 == v2 || copia_vertices.find(v2) == copia_vertices.end()) {
            continue; // v2 ya eliminado
        }
        // CONTRACCIÓN: mover todos los adyacentes de v2 a v1 ---
        ...
        // Reemplazar v2 por v1 en todos los adyacentes
